{include file='file:core/header.tpl'}
		...
		<div id="main">
			...
			<div class="mall clearfix">
				...
				<div class="mallTypeTitle"><img src="{$base_url}images/title_item_mall.png" alt="Item Mall"></div>
				<!--item mall main begin-->
				<div class="mallWrapper">
					<ul class="mallSortNav">
			{foreach item=category from=$categories}
                	<li {if $id eq $category.id}class="current"{/if}><a href="{$script_url}?act=shop&sact=mall&id={$category.id}">{$category.name}</a></li>
			{/foreach}
                    </ul>
			...
			{if $pack.vip > 0}
				{if $pack.vip <= $user.info.vip}
							    <tr>
									...
									<td><b>{if count($pack.items) > 1}{$pack.name}{else}{$pack.items[0].amount}x {$pack.items[0].info.name}{/if}</b><img src="{$base_url}images/btn_mall_detail.png" alt="Detail" class="btnDetail" style="cursor:pointer;" onclick="$('#info_{$pack.id}').slideDown();"></td>
									<td><span class="b">{if $user.info.vip > 0}<strike>{$pack.price}</strike> <b style="color:red">{$pack.price-$pack.price*$user.info.vip*5/100|floor}</b>{else}{$pack.price}{/if}</span></td>
									<td>
			...
			{literal}
				var Purchase = {
					show: function(id) {
						...
						Purchase.adjust();
						obj.load('{/literal}{$script_url}{literal}?act=purchase', {id: id,type:1});
					},
					hide: function() {
						$("#popPurchase").hide();
					...
					submit: function() {
						...
						var data = {pack_id:$(':hidden[name=item_id]', form).val(), character:$(':select[name=characters]', form).val(), amount:$(':text[name=amount]', form).val(),type:1};
						var obj = $("#popPurchaseContent").html("Loading...");

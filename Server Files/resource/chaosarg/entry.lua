...
function after_destroy_entry_chaosarg(entry)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
    Notice("Announcement: Portal to [Chaos Argent] has vanished! Enjoy!") 

end

function after_player_login_chaosarg(entry, player_name)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) --取地图入口的位置信息（地图名，坐标，目标地图名）
...
function check_can_enter_chaosarg( role, copy_mgr )
	...
	local Credit_chaosarg = GetCredit(role)
	 if Credit_chaosarg < 30 then 
...
function begin_enter_chaosarg(role, copy_mgr) 
	local	Money_Have = GetChaAttr ( role , ATTR_GD )
	local lv= GetChaAttr(role, ATTR_LV)
	local Money_Need = lv*50

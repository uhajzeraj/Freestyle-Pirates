      ...
      local closetime = GUILDWARCLOSETIME2
		for i = 1 , GUILDNOTICE2 , 1 do
			if closetime ==GUILDCLOSESHOW2[i] then
				local Notice_all = "Time remaining until the map close is"..closetime.."seconds!"
...
--		local GuildName_Blue = GetGuildName ( BlueSide_GuildID )
		...
		if Count == math.floor ( Count ) then
			...
			local Notice_map = "Goddess will be at"..SZ_Win_CountNum2.."seconds until arrival. Warriors, prepare to meet the arrival of the goddess!"
		...
		MapCopyNotice ( map_copy ,"Goddess has arrived in the Sacred War map")
...
--end
   ...
   Notice("Mini Sacred War map will be closing in 10 minutes!")
   end
   if second_five_seconds == 120 then
   Notice("Mini Sacred War Map will be closing in 5 minutes!")
   end
   if second_five_seconds == 144 then
   Notice("Mini Sacred War map will be closing in 3 minutes!")
   end

...
--									--
--------------------------------------------------------------------------
print( "Loading chaosarg Ctrl.lua" )
...
function get_map_entry_pos_chaosarg()   --设置入口的位置的坐标（坐标（米））
...
function init_entry(map)
    ...
    SetMapEntryTime(map, "2012/3/18/1/0", "0/3/0", "0/0/30", "0/0/45")	-- The first set of numbers is the starting point (Year/Month/Day/Hour/Minute), next is opening interval (Day/Hour/Minute), portal open duration (Day/Hour/Minute), and last is the map duration (Day/Hour/Minute)

end

function after_enter_chaosarg( role , map_copy )
end

function before_leave_chaosarg( role )
end

--function map_copy_first_run_chaosarg( map_copy )
--	lua_SetCopySpecialInter(map_copy , 10000)
--	Notice("10")
--
--end

--function map_copy_run_chaosarg( map_copy )
----Notice("3")
...
--每5分钟秒执行1次
function map_copy_run_special_chaosarg( map_copy ) 
...
--每5秒执行一次的
function map_run_chaosarg( map )
	
end

--地图关闭时执行
function map_copy_before_close_chaosarg( map_copy )
...
--地图关闭时执行
function map_copy_close_chaosarg( map_copy )
	ClearAllSubMapCha(map_copy)
end
--
--地图开关判断——————————————————————————————————————————

function can_open_entry_chaosarg( map ) 
...
function GiveSpecItem( role )
	...
	local Money_chaosarg = Money_all/How_Many_Active
--	SystemNotice(role,How_Many_Active)

	if How_Many_Active > 5 then
		AddMoney ( role , 0 , Money_chaosarg )
	--	local message = "Today combat has "..How_Many_Active .. "players, each being awarded "..Money_chaosarg .."'s gold"
	--	SystemNotice(role,How_Many_Active)
		return
	end
	...
	if How_Many_Active <  5 and How_Many_Active > 1 then
		...
		--local message = "You survived today's combat."..How_Many_Active .."players, each being awarded "..Money_chaosarg .."'s Gold and 1 Saint of the Ring Emblem"
		--Notice ( message )
		return
	end
	...
	if How_Many_Active == 1 then
		...
		local message = "Only survivor in today's combat is "..cha_name.."And obtained  ".. Money_chaosarg .."'s gold and 1 King of the Ring Emblem"
	--	SystemNotice(role,How_Many_Active)
